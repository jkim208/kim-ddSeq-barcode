...
def main():
    ...
    for line in filter(is_read, custom):
        ...
        if umi_f and bc_f and key_c in illumina_dict:  # check for bar code and if record is in Illumina
            ...
            if len(illumina_dict[key_c]) == 2:  # confirm that illumina record has bar code. Need to compare.
                if illumina_dict[key_c][0] != s2_bc or illumina_dict[key_c][1] != s2_umi:  # keep record if bar codes mismatch
        ...
        elif key_c in illumina_dict and (not umi_f or not bc_f):  # custom is missing a bar code or umi
            # check if Illumina does have a bar code. If so, a missing bar code in custom is significant
            if len(illumina_dict[key_c]) == 2:
                compare_dict[key_c] = illumina_dict[key_c]
                key_c = key_c + '-C'
                compare_dict[key_c] = ' '
