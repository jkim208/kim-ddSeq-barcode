...
#### Switch to hamming distance only#####################################
...
def extract_barcode(line, ref_barcode_blocks, bad_phase, bad_block, low_quality, bad_linker):
    ...
    if match_obj1:
        ...
        linker2 = regex.search(r"(TACCTCTGAGCTGAA){s<=1}", match_obj1)
        ...
        if linker1 and linker2:
            ...
            if pb in phase_blocks:
                ...
                if edit_distance(linker1, 'TAGCCATCGCATTGC') > 0:
                ...
                if edit_distance(linker2, 'TACCTCTGAGCTGAA') == 1 and linker_ed == 0:
                    # all indices after linker 2 need to shift
                    if len(linker2) > 15:  # linker length
                        shift += 1
                    elif len(linker2) < 15:
                        shift -= 1
                elif edit_distance(linker2, 'TACCTCTGAGCTGAA') == 1 and linker_ed > 0:
