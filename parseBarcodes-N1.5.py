...
def extract_barcode(line, ref_barcode_blocks, bad_phase, bad_block, low_quality, bad_linker):
    ...
    if match_obj1:
        ...
        capture = regex.search(r"^([AGCT]{0,5})([AGCT]{6})TAGCCATCGCATTGC([AGCT]{6})TACCTCTGAGCTGAA"
                               r"([AGCT]{6})ACG([AGCT]{8})GAC[T]{1,}$", match_obj1)
        if capture:
            pb = capture.group(1)
            ...
            if pb in phase_blocks:
                ...
                if bc1 and bc2 and bc3:
                    ...
                    bc2_index = match_obj1.find(bc2, 21, 32)
                    bc3_index = match_obj1.find(bc3, 42, 53)
                    umi_index = match_obj1.find(umi, 51, 64)
